...
$this->dbType = getenv('DB_DRIVER') ?? 'pdo_mysql';
$this->dbHost = getenv('DB_HOST'); // database host name
$this->dbPort  = getenv('DB_PORT') ?? 3306; // tcp port to which the database is bound
$this->dbName = getenv('DB_NAME'); // database name
$this->dbUser = getenv('DB_USER'); // database user name
$this->dbPwd  = getenv('DB_PASSWORD'); // database user password

/**
 * Shop base configuration
 */
$this->sShopURL     = 'http://' .  getenv('PROJECT_BASE_URL')  .':8000'; // eShop base url, required
// Should be removed: SSL must be standard now
// $this->sSSLShopURL  = null;            // eShop SSL url, optional
// $this->sAdminSSLURL = null;            // eShop Admin SSL url, optional

// Would point to the HTTP servers 'public web root''
$this->sShopDir     = getenv('APACHE_SERVER_ROOT') ?? __DIR__;
// Could go outside 'public_html'
$this->sCompileDir  = getenv('APACHE_SERVER_ROOT') ?? __DIR__ . '/tmp';
 // Edition of OXID eShop. options: CE|PE|EE
$this->edition = getenv('ESHOP_EDITION') ?? 'CE';
...
$this->sLogLevel = getenv('ESHOP_LOG_LEVEL') ?? 'warning';

// Debug mode for development
$this->iDebug = getenv('ESHOP_DEBUG_MODE') ?? 0;
